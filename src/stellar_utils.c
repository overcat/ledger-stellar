...
unsigned short crc16_(char *ptr, int count) {
    int crc;
    crc = 0;
    while (--count >= 0) {
        crc = crc ^ (int) *ptr++ << 8;
 ...
 */
int base32_encode_(const uint8_t *data, int length, char *result, int bufSize) {
    int count = 0;
...
void encode_key_(const uint8_t *in, char *out, uint8_t versionByte) {
    uint8_t buffer[35];
    ...
    short crc = crc16_((char *) buffer, 33);  // checksum
    buffer[33] = crc;
    buffer[34] = crc >> 8;
    base32_encode_(buffer, 35, out, 56);
    out[56] = '\0';
}

void encode_public_key(const uint8_t *in, char *out) {
    encode_key_(in, out, 6 << 3);
}

void encode_pre_auth_key(const uint8_t *in, char *out) {
    encode_key_(in, out, 19 << 3);
}

void encode_hash_x_key_(const uint8_t *in, char *out) {
...
void encode_muxed_account_(const MuxedAccount *in, char *out) {
    if (in->type == KEY_TYPE_ED25519) {
        ...
        short crc = crc16_((char *) buffer, 41);  // checksum
        buffer[41] = crc;
        buffer[42] = crc >> 8;
        base32_encode_(buffer, 43, out, 69);
        out[69] = '\0';
    }
}
